...
const confLocal = {
  ...
  originUrl: udpTsStream.originUrl,
  txParams: udpTsStream.txParams,
  sourceTimescale: udpTsStream.sourceTimescale,
  maxDurationSec: udpTsStream.maxDurationSec,
  ingestType: udpTsStream.ingestType,
  udpPort: udpTsStream.udpPort
}
...
const conf955210California = {
  ...
  signerPrivateKey: process.env.PRIVATE_KEY,
  originUrl: hlsStream.originUrl,
  txParams: hlsStream.txParams,
  sourceTimescale: hlsStream.sourceTimescale,
  maxDurationSec: hlsStream.maxDurationSec
}
...
const conf = conf955210California
...
const Test = async () => {
  try {
    ...
    console.log("\nLive offering metadata:\n" +
      curlCmd + fabLibTokenURI + "/meta/live_offering | jq")
